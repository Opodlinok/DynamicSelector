...
### Changed
- В **FeedManager** добавлены новые события:
  - `OnOfferConstructed<OfferView>` — вызывается для каждого оффера в процессе конструирования: при первичной загрузке, изменении фильтров и других пересборках.
  - `OnOfferConstructedOnce<OfferView>` — вызывается один раз для конкретного оффера при его первом создании после запуска селектора.

### Fixed
- Исправлена ошибка, при которой skeleton loading не переходил в контентное состояние, если во время загрузки офферов применялись фильтры.
...
### Added
- Добавлена возможность выводить **административные сообщения** в Inspector компонентов плагина и в мастере обновлений.

- Добавлен новый фильтр **DynamicValueToggleFilter** и компонент **DynamicOptionControl**:
  - динамически создаёт префабы с `DynamicOptionControl` (кнопки, switcher, toggle и т.д.)  
    по количеству значений в поле `FieldName`
  - поддерживает сортировку числовых значений
  - опционально автоматически подставляет соответствующее значение  
    в дочернее текстовое поле `DynamicOptionControl`

- Добавлен компонент **SwitcherGroup**, позволяющий создавать группы `Switcher`  
  и управлять ими по аналогии с `ToggleGroup`.

### Changed
> ⚠ **Breaking change**

- UI **Switcher** полностью переработан.  
  **Старое API удалено и несовместимо.**  
  Новый `Switcher` оптимизирован, гибче работает с `Graphic`-компонентами  
  и предоставляет более удобное и расширяемое API.  
  Подробнее см. **wiki плагина**.

- Полностью обновлена логика и архитектура **FeedManager**:
  - офферы создаются даже при активной фильтрации
  - загрузка не прерывается при изменении фильтров
  - список офферов больше не пересоздаётся при каждом изменении состояния фильтра
  - добавлены режимы логирования:
    - `DebugLogs`
    - `DebugLogsVerbose` (доступен только из кода)

- Обновлено поведение фильтров для соответствия новой архитектуре `Switcher`.

- В **RangeFilter** добавлена опция применения фильтра  
  **только после завершения взаимодействия с `RangeSlider`**.  
  Это позволяет существенно снизить нагрузку при фильтрации в тяжёлых проектах.

### Fixed
- Исправлено поведение **FeedManager** при `Prewarm Batch Size > 1`,  
  при котором могли появляться пустые карточки `OfferSlot`.

- Исправлено поведение мастера обновлений,  
  при котором новая версия помечалась как `unknown` до ручной проверки.

- Исправлены ошибки в логировании `OfferView`.
...
### Added
- Добавлен новый **RangeSlider** — более простой и стабильный тип двойного слайдера для фильтров.  
  - Пример префаба нового слайдера:  
    `Assets/Plugins/DynamicSelector/Sample/Data/Prefabs/Filters/RangeSlider.prefab`
- Добавлена поддержка **пакетной загрузки офферов (Prewarm)** с ограничением по времени выполнения (в миллисекундах).
  - Доступны следующие настройки:
    - `Prewarm Batch Size`
    - `Prewarm Budget Ms`
- Добавлен тестовый скрипт **`OfferSkeletonPresenter`** для реализации логики skeleton-loading, работающий напрямую с `OfferView`.

### Changed
- **`TwinSlider` помечен как устаревший (deprecated)** и заменён новым `RangeSlider`.  
  При этом `TwinSlider` остаётся частью плагина до версии **1.0** и может использоваться без ограничений.
- Обновлён механизм загрузки офферов — теперь инициализация возможна даже при свёрнутом Unity Editor или во время билда.
- Переработан доступ к данным оффера — добавлены удобные методы получения значений по строковому пути.
